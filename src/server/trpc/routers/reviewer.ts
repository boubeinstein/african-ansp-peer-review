...
export const reviewerRouter = router({
  ...
  /**
   * Get workload dashboard data for all reviewers or a specific reviewer
   * Used to monitor reviewer capacity and prevent burnout
   */
  getWorkloadDashboard: protectedProcedure
    .input(
      z.object({
    ...
    .query(async ({ ctx, input }) => {
      ...
      const reviewerWhere: Prisma.ReviewerProfileWhereInput = {
        ...(input?.reviewerId && { id: input.reviewerId }),
        ...(!input?.includeInactive && { selectionStatus: "SELECTED" }),
      };

      // Get reviewers with their assignments
      const reviewers = await ctx.db.reviewerProfile.findMany({
        where: reviewerWhere,
        include: {
          user: {
          ...
          teamAssignments: {
            where: {
              invitationStatus: { in: ["CONFIRMED", "PENDING", "INVITED"] },
            },
            include: {
              review: {
                select: {
      ...
      const workloadData = await Promise.all(
        reviewers.map(async (reviewer) => {
          // Current active assignments (IN_PROGRESS reviews)
          const currentAssignments = reviewer.teamAssignments.filter(
            (ta) =>
              ta.invitationStatus === "CONFIRMED" &&
              ["IN_PROGRESS", "REPORT_DRAFTING", "REPORT_REVIEW"].includes(
                ta.review.status
              )
          );

          // Upcoming assignments (next 30/60/90 days)
          const upcomingAssignments = {
            next30Days: reviewer.teamAssignments.filter(
              (ta) =>
                ta.invitationStatus === "CONFIRMED" &&
                ["APPROVED", "PLANNING", "SCHEDULED"].includes(ta.review.status) &&
                ta.review.plannedStartDate &&
                ta.review.plannedStartDate <= thirtyDaysFromNow &&
                ta.review.plannedStartDate > now
            ),
            next60Days: reviewer.teamAssignments.filter(
              (ta) =>
                ta.invitationStatus === "CONFIRMED" &&
                ["APPROVED", "PLANNING", "SCHEDULED"].includes(ta.review.status) &&
                ta.review.plannedStartDate &&
                ta.review.plannedStartDate <= sixtyDaysFromNow &&
                ta.review.plannedStartDate > thirtyDaysFromNow
            ),
            next90Days: reviewer.teamAssignments.filter(
              (ta) =>
                ta.invitationStatus === "CONFIRMED" &&
                ["APPROVED", "PLANNING", "SCHEDULED"].includes(ta.review.status) &&
                ta.review.plannedStartDate &&
                ta.review.plannedStartDate <= ninetyDaysFromNow &&
                ta.review.plannedStartDate > sixtyDaysFromNow
            ),
          };

          // Pending invitations
          const pendingInvitations = reviewer.teamAssignments.filter(
            (ta) => ["PENDING", "INVITED"].includes(ta.invitationStatus)
          );

          // Historical reviews (completed in last 12 months)
          const historicalReviews = await ctx.db.reviewTeamMember.count({
            where: {
          ...
          const availabilitySlots = reviewer.availabilityPeriods.filter(
            (ap) => ap.availabilityType === "AVAILABLE"
          );
          const unavailableSlots = reviewer.availabilityPeriods.filter(
            (ap) => ap.availabilityType === "UNAVAILABLE"
          );

          // Calculate workload indicator
          const activeCount = currentAssignments.length;
          const upcomingCount =
            upcomingAssignments.next30Days.length +
            upcomingAssignments.next60Days.length;
          const pendingCount = pendingInvitations.length;
          const totalWorkload = activeCount * 3 + upcomingCount * 2 + pendingCount;

          let workloadIndicator: "available" | "moderate" | "overloaded";
          if (totalWorkload === 0 && reviewer.isAvailable) {
            workloadIndicator = "available";
          } else if (totalWorkload <= 4) {
            workloadIndicator = "moderate";
          } else {
          ...
          if (availabilitySlots.length > 0) {
            const nextSlot = availabilitySlots.find((slot) => slot.startDate >= now);
            if (nextSlot) {
              nextAvailableDate = nextSlot.startDate;
            }
          }

          // Helper to get org display name
          const getOrgName = (org: { organizationCode: string | null; nameEn: string }) =>
            org.organizationCode ?? org.nameEn;
          ...
          return {
            reviewer: {
              ...
              email: reviewer.user.email,
              organization: reviewer.homeOrganization
                ? getOrgName(reviewer.homeOrganization)
                : "Unknown",
              regionalTeamId: reviewer.homeOrganization?.regionalTeamId ?? null,
              isAvailable: reviewer.isAvailable,
              isLeadQualified: reviewer.isLeadQualified,
              reviewsCompleted: reviewer.reviewsCompleted,
              reviewsAsLead: reviewer.reviewsAsLead,
            ...
            workload: {
              ...
              currentAssignments: currentAssignments.map((ta) => ({
                reviewId: ta.review.id,
                referenceNumber: ta.review.referenceNumber,
                hostOrganization: getOrgName(ta.review.hostOrganization),
                status: ta.review.status,
                phase: ta.review.phase,
                role: ta.role,
                startDate: ta.review.actualStartDate ?? ta.review.plannedStartDate,
                endDate: ta.review.actualEndDate ?? ta.review.plannedEndDate,
              })),
              upcomingAssignments: {
                next30Days: upcomingAssignments.next30Days.map((ta) => ({
                  reviewId: ta.review.id,
                  referenceNumber: ta.review.referenceNumber,
                  hostOrganization: getOrgName(ta.review.hostOrganization),
                  role: ta.role,
                  startDate: ta.review.plannedStartDate,
                  endDate: ta.review.plannedEndDate,
                })),
                next60Days: upcomingAssignments.next60Days.map((ta) => ({
                  reviewId: ta.review.id,
                  referenceNumber: ta.review.referenceNumber,
                  hostOrganization: getOrgName(ta.review.hostOrganization),
                  role: ta.role,
                ...
                next90Days: upcomingAssignments.next90Days.map((ta) => ({
                  reviewId: ta.review.id,
                  referenceNumber: ta.review.referenceNumber,
                  hostOrganization: getOrgName(ta.review.hostOrganization),
                  role: ta.role,
              ...
              pendingInvitations: pendingInvitations.map((ta) => ({
                reviewId: ta.review.id,
                referenceNumber: ta.review.referenceNumber,
                hostOrganization: getOrgName(ta.review.hostOrganization),
                role: ta.role,
                invitationStatus: ta.invitationStatus,
                startDate: ta.review.plannedStartDate,
              })),
              historicalReviewCount: historicalReviews,
            },
            availability: {
              isCurrentlyAvailable: reviewer.isAvailable,
              nextAvailableDate,
              availableSlots: availabilitySlots.slice(0, 5).map((slot) => ({
                id: slot.id,
                startDate: slot.startDate,
                endDate: slot.endDate,
                title: slot.title,
              })),
              unavailableSlots: unavailableSlots.slice(0, 5).map((slot) => ({
                id: slot.id,
                startDate: slot.startDate,
                endDate: slot.endDate,
                title: slot.title,
      ...
      const summary = {
        totalReviewers: workloadData.length,
        available: workloadData.filter((w) => w.workload.indicator === "available").length,
        moderate: workloadData.filter((w) => w.workload.indicator === "moderate").length,
        overloaded: workloadData.filter((w) => w.workload.indicator === "overloaded").length,
        totalActiveAssignments: workloadData.reduce(
          (sum, w) => sum + w.workload.currentAssignments.length,
          0
        ),
        totalUpcoming30Days: workloadData.reduce(
          (sum, w) => sum + w.workload.upcomingAssignments.next30Days.length,
          0
        ),
        averageHistoricalReviews:
          workloadData.length > 0
            ? Math.round(
                workloadData.reduce((sum, w) => sum + w.workload.historicalReviewCount, 0) /
                  workloadData.length
              )
            : 0,
      };
